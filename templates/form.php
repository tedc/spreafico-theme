...
<?php endif; ?>
	<form class="form__cell form__cell--inputs<?php if($obj != __('Richiesta di preventivo', 'sprfc')) : ?> form__cell--s6<?php endif; ?>" ng-submit="onSubmit(contactForm.$valid, '<?php echo $current_url; ?>')" name="contactForm" novalidate>
		...
		<input type="checkbox" id="privacy-<?php echo $type; ?>" required ng-model="formData.privacy" value="true" /><label for="privacy"><?php _e("Acconsento all'utilizzo dei dati inseriti secondo le finalità indicate dalla", 'sprfc'); ?> <a href="<?php ; ?>">privacy policy</a></label>
		<input type="checkbox" id="marketing-<?php echo $type; ?>" ng-model="formData.marketing" value="true" /><label for="marketing"><?php _e("Acconsento all'utilizzo dei dati inseriti per l'invio di eventuali comunicazioni di marketing da parte del Titolare del Trattamento", 'sprfc'); ?></label>
